...
for valuekey in ["geomean_slowdown", "max"]:
    pivot_df = pd.pivot_table(
        df, values=valuekey, index=["Model name"], columns=["method"]
    )
...
lp_runtime_dict = {}
for model in [
    "p32xlarge_vgg_unet_32_None",
    "p32xlarge_ResNet50_256_None",
    "p32xlarge_MobileNet_512_None",
    ...
    lp_matcher = re.compile(
        r"Solved in [0-9]+ iterations and (?P<lp_runtime>[0-9\.]+) seconds"
    )
    ilp_runtimes = []
    for path in (checkmate_data_dir() / "budget_sweep" / model / "ilp_log").glob(
        "./*.log"
    ):
    ...
    print(
        "Speedup for {} is {:0.2f} ({:.2f} versus {:.2f}, count {} vs {})".format(
            model,
            np.median(ilp_runtimes) / np.median(lp_runtimes),
            np.mean(ilp_runtimes),
            np.mean(lp_runtimes),
            len(ilp_runtimes),
            len(lp_runtimes),
        )
