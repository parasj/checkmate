...
def parse_args():
    ...
    parser.add_argument(
        "--imposed-schedule",
        default=ImposedSchedule.FULL_SCHEDULE,
        type=ImposedSchedule,
        choices=list(ImposedSchedule),
    )
...
if __name__ == "__main__":
    ...
    for B in reversed(range(4, N + 3)):  # Try several budgets
        ...
        lb_lp = lower_bound_lp_relaxation(
        ...
        try:
            ...
            if approx_lp_determinstic.schedule_aux_data:
                approx_ratio_ub = (
                    approx_lp_determinstic.schedule_aux_data.cpu
                    / lb_lp.schedule_aux_data.cpu
                )
                approx_ratio_actual = (
                    approx_lp_determinstic.schedule_aux_data.cpu
                    / ilp.schedule_aux_data.cpu
                )
